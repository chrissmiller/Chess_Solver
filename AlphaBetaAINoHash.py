...
class AlphaBetaAINoHash():
    def __init__(self, depth):
        self.color = chess.WHITE
        self.max_depth = depth
        self.nodes_visited = 0

    # Returns the move with the maximum expected value via Alpha-Beta search algorithm
    def choose_move(self, board):
        ...
        for i in range(1, self.max_depth+1):
            ...
            for move in board.legal_moves:
                board.push(move)
                val = self.min_val(board, 1, i, alpha, beta)
                alpha = max(alpha, val)
                self.nodes_visited += 1
                #print("Val for move " + str(move) + " is " + str(val) + " at depth " + str(i))
                if val > max_val:
                    max_val = val
                    best_move = move
                board.pop()
            print("At depth " + str(i) + ", Alpha-Beta Search recommends move " + str(best_move))
            print("Visited " + str(self.nodes_visited) + " nodes to produce move with value " + str(max_val))
    ...
    def max_val(self, board, curr_depth, depth_lim, alpha, beta):
        ...
        for move in board.legal_moves:
            ...
            val = self.min_val(board, curr_depth + 1, depth_lim, alpha, beta)
    ...
    def min_val(self, board, curr_depth, depth_lim, alpha, beta):
        if self.cutoff_test(board, curr_depth, depth_lim):
            return self.get_val(board)
        ...
        for move in board.legal_moves:
            ...
            val = self.max_val(board, curr_depth + 1, depth_lim, alpha, beta)
